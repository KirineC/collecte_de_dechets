...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true){
            ...
            // Aller/retour simple pour un particulier
            if (choix == 1) {
                ...
                // Calcul aller-retour avc Dijkstra
            ...
            // Tournée de particuliers avc plus proche voisin
            else if (choix == 2) {

                // On stocke une tournée par date
                ...
                while (true) {
                    ...
                    // Lecture des particuliers à visiter (max 10)
                    ...
                    // On prépare la liste de la tournée finale
                    List<Noeud> tournee = new ArrayList<>();
                    // Ensemble des particuliers encore non visités
                    Set<Noeud> nonVisites = new HashSet<>(particuliers);

                    Noeud courant = depot;
                    tournee.add(depot); // on commence toujours au dépôt

                    // On applique l'algo du plus proche voisin
                    while (!nonVisites.isEmpty()) {
                        Noeud plusProche = null;
                        double distMin = Double.MAX_VALUE;

                        // On cherche le particulier le plus proche du courant
                        for (Noeud n : nonVisites) {
                            // Dijkstra pour obtenir la distance réelle
                            List<Noeud> chemin = Dijkstra.plusCourtChemin(graphe, courant, n);
                            double dist = 0;

                            // On calcule la distance du chemin trouvé
                            for (int i = 1; i < chemin.size(); i++) {
                                Arete ar = graphe.getArete(chemin.get(i-1), chemin.get(i));
                                dist += (ar != null ? ar.getDistance() : 0);
                            }

                            // Mise à jour si plus court
                            if (dist < distMin) { distMin = dist; plusProche = n; }
                        }

                        // On ajoute le plus proche voisin à la tournée
                        tournee.add(plusProche);
                        nonVisites.remove(plusProche);
                        courant = plusProche; // on continue depuis ce noeud
                    }

                    // On ajoute le retour au dépôt
                    tournee.add(depot);

                    // On enregistre cette tournée
                    tournéesParDate.put(date, tournee);
                }

                // Affichage final de toutes les tournées par date
                System.out.println("\ntournées par date");
                for (String date : tournéesParDate.keySet()) {
                    ...
                    // Calcul de chaque segment via Dijkstra
            ...
            // Collecte complète -> on cherche un circuit eulérien
            else if (choix == 3) {
    ...
    // Affiche un trajet en utilisant Dijkstra et calcule sa distance totale
    private static double afficherTrajet(Graphe graphe, Noeud depart, Noeud arrivee, String label) {
        ...
        // On affiche chaque étape du trajet
        for (int i = 1; i < chemin.size(); i++) {
            Arete ar = graphe.getArete(chemin.get(i-1), chemin.get(i));
